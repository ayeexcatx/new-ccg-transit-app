...
- **Confirmed from code:** Incident reports can be created by Admin, CompanyOwner, and Truck users from the UI; Driver users can view incidents and add updates but do not get the “Create Incident” button. 
...
- **Confirmed from code:** Driver removal acknowledgements similarly mark unread removal notifications read and notify owners that the driver saw the removal. 
...
| Driver | **Confirmed from code:** Home, Portal, Notifications, Incidents, Profile, limited to assigned dispatch scope. | **Confirmed from code:** Incident updates; **confirmed from code:** no create-incident button in UI. | **Confirmed from code:** Their own SMS opt-in only. | **Confirmed from code:** No truck confirmation records; they acknowledge by seeing assignments/removals. | **Confirmed from code:** Can add incident updates; **needs manual verification** whether direct incident creation is intentionally blocked everywhere. | **Confirmed from code:** Driver dispatch assigned/updated/amended/cancelled/removed notifications. | **Confirmed from code:** Yes, only if owner enabled + driver opted in + valid phone. | **Confirmed from code:** No workspace switching. | **Confirmed from code:** Yes, but only for assigned dispatches/trucks. |
